# Authorizer

#### A DB-Oriented password Manager that provides the potential to fetch a password(json) in a third-party application by a URL call with the user's token and the password name.
i.e. http://servername.com/token={user_token}/name={password_name}

Also there is a management system with an Administrator Account that manages Users,Passwords and their between relationships (Access).

LoginPage
![loginpage](https://raw.githubusercontent.com/guschar92/Authorizer/master/GH_img/login.PNG)
RegisterPage
![newuserpage](https://raw.githubusercontent.com/guschar92/Authorizer/master/GH_img/new_user.PNG)
HomePage
![homepage](https://raw.githubusercontent.com/guschar92/Authorizer/master/GH_img/home.PNG)
